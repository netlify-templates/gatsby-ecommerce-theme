...
const OrderItem = (props) => {
  ...
  return (
    <div className={`${styles.root} ${collapsed === true ? styles.paddingBottom : ''}`}>
      <div className={`${headerStyling} ${styles.orderHeader}`} role={'presentation'} onClick={() => setCollapsed(!collapsed)}>
        <div className={styles.orderMeta}>
          <span className={styles.orderId}>Order #{pad(order.id, 5)}</span>
          <span className={styles.orderTotalMeta}>{`${order.items.length} products totaling `}</span>
        ...
        <div className={`${styles.toggleContainer} ${collapsed === true ? styles.rotate : ''}`}>
          <Icon symbol={'caret'} />
        </div>
      </div>
      <div className={`${styles.detailsContainer} ${collapsed === false ? styles.hide : styles.show}`}>
        <div className={styles.addressDetailContainer}>
          <div className={styles.addressContainer}>
            ...
            <span className={styles.address}>{`${order.shippingAddress?.state} ${order.shippingAddress?.postal}`}</span>
            <span className={styles.address}>{order.shippingAddress?.country}</span>
          </div>
          <div className={styles.addressContainer}>
            ...
            <span className={styles.address}>{`${order.billingAddress?.state} ${order.billingAddress?.postal}`}</span>
            <span className={styles.address}>{order.billingAddress?.country}</span>
          </div>
        </div>
        ...
        <div className={styles.transactionDetails}>
          <div className={styles.transactionalGrid}>
            <span>Subtotal:</span>
            <span><CurrencyFormatter amount={computedTotal}/></span>
            <span>GST: </span>
            <span><CurrencyFormatter amount={0}/></span>
            <span className={styles.bold}>Grand Total </span>
            <span className={styles.grandTotal}>
              <CurrencyFormatter amount={computedTotal}/></span>
          </div>
        </div>
      </div>
    </div>
