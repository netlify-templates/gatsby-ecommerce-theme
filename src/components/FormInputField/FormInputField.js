    ...
    <div className={`formField ${styles.formField}`}>
      ...
      {(type === 'textarea') && (
        <textarea
          ...
          value={value}
          pattern={pattern}
          className={`${styles.textarea} ${ icon ? styles.conditionalIconPadding : ''} ${error ? styles.fieldRequired : ''}`}
          onChange={e => handleChange(id, e.target.value)}
          disabled={disabled}
          ref={ref}
        /> 
      ...
      {icon && (
        <div className={`${styles.iconContainer} ${labelName !== undefined ? styles.offsetIcon : ''}`}>
          <Icon symbol={icon} />
        </div>
      )}
    </div>
